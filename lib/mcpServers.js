...
export async function executeGrabMapsCalculateRoute(params = {}) {
  ...
  let origin = { latitude: 1.3000, longitude: 103.8400, name: "Driver Current Location (Central)" };
  if (typeof params.origin === "string") {
    origin.name = params.origin;
    if (params.origin.toLowerCase().includes("jurong")) {
      origin = { latitude: 1.3331, longitude: 103.7423, name: params.origin };
    } else if (params.origin.toLowerCase().includes("changi") || params.origin.toLowerCase().includes("airport")) {
      origin = { latitude: 1.3553, longitude: 103.9870, name: params.origin };
    } else if (params.origin.toLowerCase().includes("woodlands")) {
      origin = { latitude: 1.4369, longitude: 103.7865, name: params.origin };
    }
  } else if (params.origin && typeof params.origin === "object") {
    origin = {
      latitude: params.origin.latitude || params.origin.lat || 1.3000,
      longitude: params.origin.longitude || params.origin.lng || 103.8400,
      name: params.origin.name || "Specified Origin"
    };
  }

  // Resolve destination safely
  let destination = { latitude: 1.3040, longitude: 103.8318, name: "ION Orchard Taxi Stand F12" };
  if (typeof params.destination === "string") {
    destination.name = params.destination;
    if (params.destination.toLowerCase().includes("jurong")) {
      destination = { latitude: 1.3331, longitude: 103.7423, name: params.destination };
    } else if (params.destination.toLowerCase().includes("changi") || params.destination.toLowerCase().includes("airport")) {
      destination = { latitude: 1.3553, longitude: 103.9870, name: params.destination };
    } else if (params.destination.toLowerCase().includes("bayfront") || params.destination.toLowerCase().includes("mbs")) {
      destination = { latitude: 1.2840, longitude: 103.8607, name: params.destination };
    }
  } else if (params.destination && typeof params.destination === "object") {
    destination = {
      latitude: params.destination.latitude || params.destination.lat || 1.3040,
      longitude: params.destination.longitude || params.destination.lng || 103.8318,
      name: params.destination.name || "Specified Destination"
    };
  }
